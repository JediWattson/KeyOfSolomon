...
export default async function handler(req, res) {
    ...
    const promisePlanets = planets.map(async planet => {
        const metaLocRes = await fetch(`https://images-api.nasa.gov/metadata/${planet.nasaId}`)
        const metaLoc = await metaLocRes.json();
        const metaDataRes = await fetch(metaLoc.location);
        const metaData = await metaDataRes.json()
        const { ["AVAIL:Title"]: imgTitle, ["File:ImageHeight"]: height, ["File:ImageWidth"]: width, ["AVAIL:Description"]: description } = metaData;
